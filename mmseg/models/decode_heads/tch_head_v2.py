...
class TchHeadV2(BaseDecodeHead):
    def __init__(self, pool_scales=(1, 2, 3, 6),dilations=(1, 6, 12, 18), **kwargs):
        ...
        self.aspp_dwon_chs = ConvModule(self.channels*4,self.channels,kernel_size=3,stride=1,padding=1,
        ...
        self.tch_dwon_chs = ConvModule(self.channels*2,self.channels,kernel_size=3,stride=1,padding=1,
                                        conv_cfg=self.conv_cfg,norm_cfg=self.norm_cfg,act_cfg=self.act_cfg)
        #############增加ASPPmodule 结束###################    
        # FPN Module
