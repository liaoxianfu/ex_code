...
class SwinTransformerConv7(BaseModule):
    ...
    def __init__(self,
        ...
        super(SwinTransformerConv7, self).__init__(init_cfg=init_cfg)
        ...
        self.dims = dims = self.num_features
    ...
    #############################自定义模型开始###########################################
        ...
        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, sum(conv_depths))]
        cur = 0
        for i in range(4):
            stage = nn.Sequential(*[
        ...
        for i in range(4):
            merge_layer = nn.Conv2d(dims[i] * 2, dims[i], kernel_size=1, stride=1)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerConv7, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        conv_x = x.view(-1, *hw_shape, self.dims[0]).permute(0, 3, 1, 2).contiguous()

        outs = []
        for i, stage in enumerate(self.stages):
            conv_x = self.downsample_layers[i](conv_x)
            conv_x = self.conv_stages[i](conv_x)
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape)
            out = out.view(-1, *out_hw_shape, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
            conv_x = torch.cat((conv_x, out), dim=1)
            conv_x = self.merge_layers[i](conv_x)
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                B, C, H, W = conv_x.shape
                conv_x = conv_x.view(B, C, H * W).permute(0, 2, 1).contiguous()
                conv_x = norm_layer(conv_x)
                conv_x = conv_x.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
                outs.append(conv_x)
        # print(len(outs))
        return outs
