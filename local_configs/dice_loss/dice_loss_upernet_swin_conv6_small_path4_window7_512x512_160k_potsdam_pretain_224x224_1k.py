_base_ = [
    '../_base_/models/upernet_swinconv6.py', '../_base_/datasets/potsdam.py', 
...
model = dict(backbone=dict(init_cfg=dict(type='Pretrained', checkpoint=checkpoint_file),
            ...
            decode_head=dict(
                    loss_decode=[dict(type='CrossEntropyLoss', loss_name='loss_ce', loss_weight=1.0),
                        ...
                        in_channels=[96, 192, 384, 768], num_classes=6
                    ),
            auxiliary_head=dict(
                    loss_decode=[dict(type='CrossEntropyLoss', loss_name='loss_ce',loss_weight=1.0),
                        dict(type='DiceLoss', loss_name='loss_dice', loss_weight=3.0)],
                        in_channels=384, num_classes=6
                    )
            )
...
data = dict(samples_per_gpu=2, test=dict(img_dir='img_dir/test', ann_dir='ann_dir/test'))
